...
while($row=mysqli_fetch_array($query))
{
	...
	$sql = "SELECT users.*, users_orders.* FROM users INNER JOIN users_orders ON users.u_id=users_orders.u_id WHERE users_orders.rs_id='" . $_SESSION["res_id"] . "'";
	$query=mysqli_query($db,$sql);
	if(!mysqli_num_rows($query) > 0 ) {
		echo '<td colspan="8"><center>No Orders</center></td>';
	} else {				
		while($rows=mysqli_fetch_array($query)) {
			$total = $rows['quantity'] * $rows['price']; // calculate total for each row
			...
			if($status=="" or $status=="NULL") {
				echo '<td> <button type="button" class="btn btn-info" style="font-weight:bold;"><span class="fa fa-bars"  aria-hidden="true" > Dispatch</button></td>';
				...
				echo '<td> <button type="button" class="btn btn-success" ><span  class="fa fa-check-circle" aria-hidden="true"> Delivered</button></td>';
...
?>
                        ...
                        <button class="btn btn-warning" style="background-color:#ffca93;color:#000000;border:none;margin-left:15px;margin-top:15px;font-size:15px;" type="button" onclick="location.href='export_all_orders.php'">Export Data</button>
						 </div>
